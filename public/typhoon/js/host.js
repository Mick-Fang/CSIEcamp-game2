...
async function syncStateToServer(clearActions = false) {
    ...
    await fetch('/csiecamp_game2/typhoon/api/state', {
        method: 'POST',
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loginView && loginBtn) {
        ...
        pwdInput.addEventListener("keyup", e => { if (e.key === "Enter") doLogin(); });
    }
    ...
    document.getElementById("setup-form").addEventListener("submit", async e => {
        ...
        if (serverReadyCount < 5) {
            alert(`⚠️ 請等待所有小隊按下準備好了！\n目前 ${serverReadyCount}/5 小隊已準備。`);
            return;
        }
        const names = [];
        for (let i = 0; i < 5; i++) names.push(e.target.elements[`team-${i}`].value.trim());
...
function render() {
    ...
    if (state.phase === "SETUP") {
        ...
        if (serverReadyCount >= 5) {
            statusText.textContent = `所有小隊已準備完畢！ (5/5)`;
            statusText.style.color = "var(--success-green)";
            startBtn.style.opacity = "1";
            startBtn.style.pointerEvents = "auto";
            startBtn.disabled = false;
        } else {
            statusText.textContent = `正在等待所有小隊準備... (${serverReadyCount}/5)`;
